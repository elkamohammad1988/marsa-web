...
its REST API with Postgres row-level security, and 1,560 unit tests behind a
...
| Unit tests | **1560 / 1560 passing**, 47 files |
...
Seven, in the order the product makes sense in. All seven are regenerated by
...
**Every one carries the concept-build disclosure**, because the site does. The
capture script used to hide it — reasonable-sounding, since the same badge in
every frame is chrome, and wrong here: a portfolio image is the only place this
product is seen with no site around it, no navigation and no badge to open. What
that produced was a hero image showing a European IBAN, a €12,480.55 balance and
an "Open An Account" button with every marker that this is a concept removed by
the script itself. `requireDisclosure()` now throws rather than write an unmarked
image, and `tests/portfolio-honesty.test.ts` fails if that rule is softened back.

The script does not scroll to a hand-tuned offset. It searches for a scroll
position that **bisects no line of text and leaves nothing meaningful under the
floating navbar**, and reports the position it chose. The tuned-offset version is
why an earlier set sliced a heading in half at the top edge of two images: an
offset that clears one page's header lands in the middle of the next page's
paragraph, so each fix moved the cut somewhere else rather than removing it.

That search had a hole, and the converter shot fell through it. Boxes taller than
240 px were skipped as layout containers rather than lines — but
`/tools/currency-converter` sets its `<h1>` at display size and it wraps to five
lines, so the one element that mattered most was the one the scorer never saw,
and every candidate position scored as if slicing it were free. Headings are now
exempt from that ceiling: a heading is never a layout container, and excluding it
by height was excluding it by importance, backwards.
...
Two images are gone rather than fixed. `08-pricing` photographed the concept
product's own subscription tiers, and an image whose caption has to explain that
€4.99 a month is not a rate card is an image doing negative work. `07-mobile` was
the landing page at 390 px — the same hero as `01`, narrower — which proved the
breakpoint and nothing else; it is now the demo mid-flow, because the claim worth
making about mobile is that the whole interaction works down there.
